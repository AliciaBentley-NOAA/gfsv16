...
cat > ${DATA_PATH}/${CASE}/${CYCLE}/plot_llvl_${REGIONNAME}_${CYCLE}.csh <<EOF
#!/bin/csh
#SBATCH --account=ovp
...
sbatch ${DATA_PATH}/${CASE}/${CYCLE}/plot_llvl_${REGIONNAME}_${CYCLE}.csh
sleep 5
...
cat > ${DATA_PATH}/${CASE}/${CYCLE}/plot_llvl_${DOMAIN}_${CYCLE}.sh <<EOF
#!/bin/bash
#SBATCH --account=ovp
...
ncl 'scriptyyyymmddhh="${CYCLE}"' 'eventname="${CASE}"' 'scriptregion="${DOMAIN}"' 'fhr_inc="${FHR_INC}"' 'fhr_start="${FHR_START}"' 'fhr_end="${FHR_END}"' 'images_path="${IMAGES_PATH}"' ${DATA_PATH}/${CASE}/${CYCLE}/plot_gfs_lambert_lowcloud.ncl
...
sbatch ${DATA_PATH}/${CASE}/${CYCLE}/plot_llvl_${DOMAIN}_${CYCLE}.sh
sleep 5
